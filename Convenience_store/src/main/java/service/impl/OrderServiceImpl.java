...
public class OrderServiceImpl implements OrderService {
    ...
    public Invoice placeOrder(int storeId, int employeeId, List<OrderService.CartItem> cart) throws SQLException {
        if (cart == null || cart.isEmpty()) throw new IllegalArgumentException("Giỏ hàng trống");
        ...
        for (OrderService.CartItem item : cart) {
            if (item.quantity <= 0) throw new IllegalArgumentException("Số lượng phải > 0");
            ProductUnit unit = unitRepo.findById(item.unitId).orElseThrow(() -> new IllegalArgumentException("Không tìm thấy đơn vị id=" + item.unitId));
            int baseQty = unit.getRatio() * item.quantity;
            inventoryRepo.findByStoreAndProduct(storeId, unit.getProductId()).filter(inv -> inv.getQuantity() >= baseQty).orElseThrow(() -> new IllegalStateException("Không đủ tồn kho cho sản phẩm id=" + unit.getProductId() + " (cần " + baseQty + " base unit)"));
            BigDecimal subtotal = unit.getSellingPrice().multiply(BigDecimal.valueOf(item.quantity));
            total = total.add(subtotal);
            InvoiceDetail d = new InvoiceDetail(); d.setProductId(unit.getProductId()); d.setUnitId(item.unitId); d.setQuantity(item.quantity); d.setPriceAtSale(unit.getSellingPrice()); d.setSubtotal(subtotal); details.add(d);
        }
        Invoice invoice = new Invoice(); invoice.setStoreId(storeId); invoice.setEmployeeId(employeeId); invoice.setTotalAmount(total);
        orderRepo.createOrder(invoice, details);
        return invoice;
    }

    @Override public void cancelOrder(int invoiceId, int storeId) throws SQLException { storeRepo.findById(storeId).orElseThrow(() -> new IllegalArgumentException("Không tìm thấy cửa hàng id=" + storeId)); orderRepo.cancelOrder(invoiceId, storeId); }
